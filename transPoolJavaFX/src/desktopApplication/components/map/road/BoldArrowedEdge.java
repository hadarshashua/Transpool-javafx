package desktopApplication.components.map.road;
...
public class BoldArrowedEdge extends Edge {

    private final double ARROW_HEAD_SIZE = 15;
    private EdgeGraphic edgeGraphic;
    private String edgeStyleSheet;
    private Polygon arrowHead;
    private Polygon secondArrowHead;
    private boolean isOneWay;

    public BoldArrowedEdge(ICell source, ICell target, boolean isOneWay) {
        super(source, target);
    ...
    public EdgeGraphic getGraphic(Graph graph) {
        if (edgeGraphic == null) {
            ...
            edgeGraphic.getLine().getStyleClass().add("boldLine");
            edgeGraphic.getText().getStyleClass().add("edge-text");
        }
        return edgeGraphic;
    }
...
//        double arrowHeadY = startY + ((endY - startY) / 2);
        ...
        if(!isOneWay)
            recalculateOppositeArrowHead(endX, endY, startX, startY);
    }

    private void recalculateOppositeArrowHead(double startX, double startY, double endX, double endY)
    {
        double arrowHeadY;
        double arrowHeadX;
        ...
        // remove last arrow head and add this new one
        Group group = edgeGraphic.getGroup();
        group.getChildren().remove(secondArrowHead);

        // create and add the new arrow head
        secondArrowHead = new Polygon(arrowHeadX, arrowHeadY, x1, y1, x2, y2);
        secondArrowHead.getStyleClass().add("arrow-head");
        group.getChildren().add(secondArrowHead);
    }
